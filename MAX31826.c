...
static const OW_temp_props_t MAX31826_temp_props = {
	MAX31826_convTimes, MAX31826__RES_12BIT, MAX31826__GRANULARITY, 0 };	//!< MAX31826 temperature sensor parameters
...
__WEAK FctERR NONNULL__ MAX31826_Init_Sequence(MAX31826_t * const pCpnt)
{
	...
	if (!err)
	{
		...
		pCpnt->temp.resIdx = MAX31826__RES_12BIT;
	}
...
__STATIC FctERR NONNULL__ MAX31826_EEP_Read_Scratchpad(OW_eep_t * const pEEP)
{
	...
	const uint8_t cmd[2] = { OW_EEP__READ_SCRATCHPAD, LOBYTE(pEEP->pScratch->address) };
	OWWrite(pDrv, cmd, sizeof(cmd));

	OWRead(pDrv, pEEP->pScratch->pData, pEEP->props.scratchpad_size);
	OWRead(pDrv, &crc, sizeof(crc));

	OW_set_busy(pSlave, false);

	pEEP->pScratch->crc = crc;

	crc = 0U;
	OWCompute_DallasCRC8(&crc, cmd, sizeof(cmd));
	OWCompute_DallasCRC8(&crc, pEEP->pScratch->pData, pEEP->props.scratchpad_size);

	if (crc != pEEP->pScratch->crc) { err = ERROR_CRC; }
...
__STATIC FctERR NONNULL__ MAX31826_EEP_Write_Scratchpad(OW_eep_t * const pEEP, const uint8_t * pData, const uint32_t addr, const size_t len)
{
	...
	OWRead(pDrv, &CRC_received, sizeof(CRC_received));
...
FctERR NONNULL__ MAX31826_Write_Memory(MAX31826_t * const pCpnt, const uint8_t * pData, const uint32_t addr, const size_t len)
{
	...
	while (data_len != 0U)
	{
		...
		if (unaligned_len != 0U)
		{
			...
			err = MAX31826_Read_Memory(pCpnt, pEEP->pScratch->pData, address, read_len);
		...
		else if (write_len < pEEP->props.scratchpad_size)
		{
			...
			err = MAX31826_Read_Memory(pCpnt, &pEEP->pScratch->pData[write_len], address + write_len, read_len);
		...
		err = MAX31826_EEP_Write_Scratchpad(pEEP, pEEP->pScratch->pData, address, pEEP->props.scratchpad_size);
		if (err != ERROR_OK)	{ goto ret; }
